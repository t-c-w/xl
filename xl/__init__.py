...
def parse_organic_result(search_ires_li_instance):
    ...
    if div_class_s:
        div_class_s = div_class_s[
            -1
        ]  # take the last one only (hopefully the one just after the last class='r')
        cite_text = div_class_s.find(name='cite', attrs={})
        if cite_text:
            d[
                'cite_text'
            ] = cite_text.text  # link attached to the title of the organic result
            d['domain'] = tldextract.extract(
                d['cite_text'].split(' ')[0]
            ).domain  # domain of this link

        f_slp_text = div_class_s.find(name='div', attrs={'class': 'f slp'})
        if f_slp_text:
            d['f_slp_text'] = f_slp_text.text
        ...
        if osl:
            d['osl'] = osl.renderContents()

        _orp_table = div_class_s.find(name='table', attrs={})
        if _orp_table:
            table_tds_list = _orp_table.findAll('td')
            table_tds_list = [
                x for x in table_tds_list if x.text
            ]  # keep only the tds that have text
...
def parse_ad(rad):
    ...
    if ad_text_html:
        ...
        ad_text_lines = [
            re.sub(r'</?b>', '', x) for x in d['ad_text_html'].split('<br/>')
        ]
...
if __name__ == '__main__':
    ...
    file_list = [
        'www hidden camera in hotel com.html',
